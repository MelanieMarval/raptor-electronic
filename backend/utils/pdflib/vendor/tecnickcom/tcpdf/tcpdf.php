...
class TCPDF {
	 ...
	 *  $pdflib->startTransaction();
	 *  // store starting values
	 *  $start_y = $pdflib->GetY();
	 *  $start_page = $pdflib->getPage();
	 *  // call your printing functions with your parameters
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  $pdflib->MultiCell($w=0, $h=0, $txt, $border=1, $align='L', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0);
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  // get the new Y
	 *  $end_y = $pdflib->GetY();
	 *  $end_page = $pdflib->getPage();
	 *  // calculate height
	 *  $height = 0;
	 ...
	 *  $pdflib = $pdflib->rollbackTransaction();
	 *
	 * @param $w (float) Width of cells. If 0, they extend up to the right margin of the page.
	 * @param $txt (string) String for calculating his height
	 * @param $reseth (boolean) if true reset the last cell height (default false).
	 ...
	 * @see Close()
	 */
	public function Output($name='doc.pdflib', $dest='I') {
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					header('Content-Type: application/pdflib');
					if (headers_sent()) {
			...
			case 'D': {
				...
				if (strpos(php_sapi_name(), 'cgi') === false) {
					...
					header('Content-Type: application/pdflib', false);
				} else {
					header('Content-Type: application/pdflib');
				}
			...
			case 'E': {
				...
				$retval = 'Content-Type: application/pdflib;'."\r\n";
				$retval .= ' name="'.$name.'"'."\r\n";
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									if (empty($parsedUrl['scheme']) AND (strtolower(substr($parsedUrl['path'], -4)) == '.pdflib')) {
	 ...
	 * (see Chapter 14.3.3 Document Information Dictionary of PDF32000_2008.pdflib Reference)
	 * @return int object id
	 * @protected
	 */
	...
	protected function _putXMP() {
		...
		$xmp .= "\t\t\t".'<dc:format>application/pdflib</dc:format>'."\n";
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		...
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		$xmp .= "\t\t".'<rdf:Description rdf:about="" xmlns:pdflib="http://ns.adobe.com/pdflib/1.3/">'."\n";
		$xmp .= "\t\t\t".'<pdflib:Keywords>'.TCPDF_STATIC::_escapeXML($this->keywords).'</pdflib:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdflib:Producer>'.TCPDF_STATIC::_escapeXML(TCPDF_STATIC::getTCPDFProducer()).'</pdflib:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		...
		$xmp .= "\t\t\t\t\t\t".'<pdfaSchema:namespaceURI>http://ns.adobe.com/pdflib/1.3/</pdfaSchema:namespaceURI>'."\n";
		$xmp .= "\t\t\t\t\t\t".'<pdfaSchema:prefix>pdflib</pdfaSchema:prefix>'."\n";
